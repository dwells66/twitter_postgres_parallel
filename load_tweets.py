...
def insert_tweet(connection,tweet):
    ...
    with engine.begin() as connection:
        ...
        sql = sqlalchemy.sql.text('''
        INSERT INTO users
            (id_users, created_at, updated_at, friends_count, listed_count, statuses_count, protected, verified, screen_name, name, location, description, withheld_in_countries, id_urls)
        VALUES
            (:id_users, :created_at, :updated_at, :friends_count, :listed_count, :statuses_count, :protected, :verified, :screen_name, :name, :location, :description, :withheld_in_countries, :id_urls)
            ON CONFLICT DO NOTHING
            ''')
        ...
        VALUES
            (:id_tweets, :id_users, :created_at, :in_reply_to_status_id, :in_reply_to_user_id, :quoted_status_id, :retweet_count, :favorite_count, :quote_count, :withheld_copyright, :withheld_in_countries, :source, :text, :country_code, :state_code, :lang, :place_name,
             CASE WHEN :geo IS NOT NULL THEN ST_GeomFromText(:geo,4326) ELSE NULL END)
            ''')
        connection.execute(sql, {
            'id_tweets': tweet['id'],
            'id_users': tweet.get('user', {}).get('id'),
